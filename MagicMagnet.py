...
def main():
    ...
    if empty_params == len(args.__dict__):
        ...
        if args.output_file==None:
            parameters["output_file"]=parameters["value_to_search"]+".json"
        else:
            if "." in args.output_file:
                parameters["output_file"]=args.output_file
            else:
                parameters["output_file"] = args.output_file+".json"
        if args.dont_save != None:
        ...
        if not parameters["dont_save"]:
            json.dump(process.links,open(parameters["output_file"],"w+"))
